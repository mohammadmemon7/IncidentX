...
const IngestLogs = () => {
  ...
  const fetchLogs = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:3000/api'}/ingest/logs`, {
  ...
  const handleSimulate = async () => {
    ...
    try {
      await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:3000/api'}/ingest`, demoPayload, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
