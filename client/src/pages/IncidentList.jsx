...
const IncidentRow = ({ incident }) => (
  ...
  >
    <div className="flex items-center gap-6 flex-1 min-w-0">
      <div className={`shrink-0 w-14 h-14 rounded-lg flex items-center justify-center ${incident.status === 'resolved' ? 'bg-green-500/10 text-green-500' :
      ...
      <div className="flex-1 min-w-0 space-y-1.5">
        <div className="flex items-center gap-4">
          ...
          <span className={`text-xs font-black uppercase tracking-widest px-2.5 py-1 rounded-full border ${incident.severity === 'critical' ? 'border-red-500/20 text-red-400 bg-red-500/5' :
        ...
        <div className="flex items-center gap-3 w-full lg:w-auto overflow-x-auto pb-2 lg:pb-0 scrollbar-hide">
          {['all', 'investigating', 'identified', 'resolved'].map((filter) => (
            <button
              ...
              className={`h-10 px-5 rounded-lg text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeFilter === filter
